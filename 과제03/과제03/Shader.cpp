...
{
	return(CompileShaderFromFile(L"Shader.hlsl", "VSMain", "vs_5_1", ppd3dShaderBlob));
}

D3D12_SHADER_BYTECODE CShader::CreatePixelShader(ID3DBlob **ppd3dShaderBlob) //픽셀 셰이더 바이트 코드를 생성(컴파일)한다. 
{
	return(CompileShaderFromFile(L"Shader.hlsl", "PSMain", "ps_5_1", ppd3dShaderBlob));
}


D3D12_SHADER_BYTECODE CShader::CompileShaderFromFile(const WCHAR *pszFileName, LPCSTR pszShaderName,
	LPCSTR pszShaderProfile, ID3DBlob **ppd3dShaderBlob) //셰이더 소스 코드를 컴파일하여 바이트 코드 구조체를 반환한다. 
{
...
void CShader::CreateShader(ID3D12Device *pd3dDevice, ID3D12RootSignature *pd3dRootSignature) //그래픽스 파이프라인 상태 객체를 생성한다. 
{
